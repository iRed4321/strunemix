...
fn name_array() {
    ...
    assert_eq!(Person::<String>::as_name_array(), [name, age, note]);
}
...
fn data_array() {
    ...
    assert_eq!(person.to_data_array(), [name, age, note]);
}

#[test]
fn from_enum_name() {
    let age = PersonAttrName::Age;
    assert!(age.get_str() == "age");
}
...
#[test]
fn form_empty() -> Result<(), StrunemixError> {
